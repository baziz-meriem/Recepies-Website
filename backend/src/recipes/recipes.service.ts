...
import { mergeRecipeVideoUrls } from './recipe-videos.util';
...
export class RecipesService {
  ...
  private normalizeVideoInput(dto: {
    video?: string;
    videos?: string[];
  }): string[] {
    const out: string[] = [];
  ...
  async findOneWithDetails(id: number) {
    ...
    return {
      ...
      videos: mergeRecipeVideoUrls(recette),
    };
  }

  /** Détail brut pour l’admin (brouillons inclus), sans étapes. */
  ...
  create(dto: CreateRecetteDto) {
    const vids = this.normalizeVideoInput(dto);
  ...
  async update(id: number, dto: UpdateRecetteDto) {
    ...
    if (dto.videos !== undefined) {
      const list = dto.videos.map((s) => String(s).trim()).filter(Boolean);
      row.videos = list.length ? list : null;
      row.video = list[0] ?? null;
    } else if (dto.video !== undefined) {
      const v = String(dto.video).trim();
      row.video = v || null;
      row.videos = v ? [v] : null;
    }
