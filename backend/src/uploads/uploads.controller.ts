...
export class UploadsController {
  ...
  @UseInterceptors(
    FileInterceptor('file', {
      ...
      fileFilter: (_req, file, cb) => {
        if (!/^video\/(mp4|webm|quicktime|mpeg)$/i.test(file.mimetype)) {
          cb(
            new BadRequestException(
              'Format non pris en charge (MP4, WebM, MOV).',
            ),
            false,
          );
          return;
      ...
      storage: diskStorage({
        destination: (_req, _file, cb) => {
          ...
          const dir = join(root, 'videos', UPLOAD_SUBDIR);
          if (!existsSync(dir)) {
        ...
        filename: (_req, file, cb) => {
          ...
          const safe = ['mp4', 'webm', 'mov'].includes(raw) ? raw : 'mp4';
          cb(
            null,
  ...
  uploadVideo(@UploadedFile() file: Express.Multer.File | undefined) {
    if (!file) {
      throw new BadRequestException('Aucun fichier reçu.');
    }
    const rel = `videos/${UPLOAD_SUBDIR}/${file.filename}`;
    return {
      path: rel,
      url: `/static/${rel}`,
    };
